...
buildSpatialDataset <- function(host, domain, doc, ids = NULL,
  ...
  refs = doc
  if(!is.null(ids)){
    refs <- doc[sapply(doc, function(x){x$inventoryId %in% ids})]
  }
    ...
    capture.output({
      ...
      if(exportPartialResults){
        if(!is.null(out)){
          ...
          filename <- paste0(domain, "_", ref$inventoryId)
      ...
      if(!is.null(out)){
        ...
        if(exportPartialResults){
          if(!is.null(out.points)){
            ...
            filename <- paste0(domain, "_point_", ref$inventoryId)
            esf::st_write(out, file.path(exportPath, paste0(filename, ".shp")))
          }
        }
      }
    }, file = paste0(paste("log", domain, ref$inventoryId, sep="_"), ".txt"))
